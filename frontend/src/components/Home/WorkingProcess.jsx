...
const WorkingProcess = () => {
    ...
    return (
        <div id="Working" className="px-4 sm:px-6 md:px-10 lg:px-20 py-10">
            ...
            <div className="space-y-4">
                {steps.map((step, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={`relative max-w-3xl mx-auto border-b-4 rounded-2xl border transition-all duration-300 ${isOpen ? 'bg-[#5465ff] text-white' : 'bg-white text-black'
                                }`}
                        >
                            <button
                            ...
                            >
                                ...
                                <span
                                    className={`w-6 h-6 flex items-center justify-center rounded-full border ${isOpen ? 'border-white text-white' : 'border-black text-black'
                                        }`}
                                >
                                    {isOpen ? '-' : '+'}
                                </span>
