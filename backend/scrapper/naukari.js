...
async function scrapeNaukriJobs(keyword) {
  ...
  try {
    ...
    const url = `https://www.naukri.com/${keyword.replace(
      /\s+/g,
      "-"
    )}-jobs`;
    ...
    const jobs = await page.evaluate(() => {
      ...
      jobElements.slice(0, 15).forEach((element) => {
        try {
          ...
          console.log(
            "Error parsing Naukri job element:",
            error.message
          );
        }
      });
    ...
    if (jobs.length === 0) {
      ...
      console.log(
        `✅ Naukri jobs scraped: ${jobs.length}`
      );
    }
    ...
    if (browser) {
      try {
        ...
        console.log(
          "⚠️ Error closing Naukri browser:",
          error.message
        );
      }
    }
