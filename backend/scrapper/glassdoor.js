  ...
  try {
    ...
    const jobs = await page.evaluate(() => {
      ...
      jobCards.forEach((card, index) => {
        ...
        try {
          ...
          let title =
            lines.find(
              (line) => line.length > 10 && line.length < 100
            ) || "N/A";

          let company =
            lines.find(
              (line) =>
                line.includes("Company") || line.length < 50
            ) || "Unknown";
          ...
          console.log(
            "Error processing Glassdoor job card:",
            error.message
          );
        }
      });
    ...
    if (browser) {
      try {
        ...
        console.log(
          "⚠️ Error closing Glassdoor browser:",
          error.message
        );
      }
    }
